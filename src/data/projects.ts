const BASE_IMG_PATH = `${import.meta.env.BASE_URL}img/`;
...
[배포] AWS EC2 + Nginx 조합으로 프로덕션 배포`,
    ...
    architectureAlt: 'Send.U.Scent 아키텍처 이미지',
    achievements: [
...
[실시간 발송 (Event-Driven)] 학습자의 클릭, 상태 변화 등 이벤트 발생 시 즉시 발송하며 공통 발송 메서드로 조건 검증 및 맞춤 메시지 생성
[배치 발송 (Batch-Scheduled)] Spring Batch 기반으로 정해진 시점에 대상 사용자를 일괄 조회 및 발송
[메시지 생성] Builder 패턴으로 필수/선택 파라미터 구분, 템플릿 분리 + 치환문자 방식으로 유지보수성 및 확장성 향상
[저장소] MSSQL 기반 학습자 수강 데이터, 발송 이력, 조건 설정 관리`,
    architectureImage: `${BASE_IMG_PATH}[project]알림스케쥴링 아키텍처.png`,
...
Unity 기반의 3D 메타버스에서 강사와 학생이 아바타로 상호작용하며, OpenVidu를 통한 실시간 화상 강의와 화면 공유 기능을 제공합니다. Vue.js 기반 관리 시스템과 Django 백엔드로 강의 스케줄, 사용자 관리, 결제 등을 처리합니다.`,
    architecture: `프론트엔드(Vue.js 3) ↔ REST API(Django) ↔ MySQL 다층 구조로 설계되었습니다.

[메타버스 환경] Unity 기반 3D 메타버스 + Photon 서버로 멀티플레이 실시간 통신 구현
[화상 강의] OpenVidu CE v2.20.0 기반 WebRTC 화상 수업 + 화면 공유 + 채팅 기능
[관리 시스템] Vue3 + bootstrap-vue 기반 강사/학생 관리 및 강의 스케줄 시스템
[실시간 통신] Photon으로 메타버스 사용자 움직임 동기화, OpenVidu로 화상 강의 스트리밍
[배포] AWS EC2 + Nginx + Jenkins 기반 CI/CD, OpenVidu-Server Docker 커스터마이징으로 SSL 인증서 적용`,
    architectureImage: 'https://user-images.githubusercontent.com/32251962/142543897-bcc09e55-98fb-4479-8933-36b6c5fb72c9.png',
    architectureAlt: '1Dayverse 아키텍처 다이어그램',
    achievements: [
      'OpenVidu CE v2.20.0 On-Premises 환경을 활용하여 WebRTC 기반 화상 교육 기능 구현',
      'OpenVidu-Server를 커스텀 도커라이징하여 SSL 인증서 설정 및 운영 환경에 맞는 Docker Compose 구조 구성',
      'OpenVidu 설치, SSL 인증서 적용, 테스트 환경 구축 과정을 공식 문서 기반으로 검증 및 문서화하여 부트캠프 교육생들에게 공유하고 WebRTC 기술 도입 및 학습 효율 향상에 기여',
      'Vue3 + bootstrap-vue 기반 사용자 캠, 채팅, 화면 공유 컴포넌트 구현',
      'Vue 기반 메타버스 진입 메인 화면을 구현하고 Unity 환경과 연결되는 사용자 진입 흐름 설계',
      'Figma 기반 웹/메타버스 UI 디자인 시스템을 구성하고 화면 구현',
      'Jenkins, AWS EC2, Nginx 기반 CI/CD 및 HTTPS 배포 환경을 구축하여 GitLab 메인 브랜치 반영 시 자동 배포 환경 구성',
    ],
  },
];
